# 笔记总索引

这里是整个笔记库的人用入口。它的作用是帮我快速找到主题、项目、资料和复盘，不追求一次性整理完所有内容。

## 目录入口

- [topics](../topics/README.md)：通用知识笔记。
- [projects](../projects/README.md)：长期项目、产品和系统设计。
- [reference](../reference/README.md)：外部资料原文。
- [inbox](../inbox/README.md)：临时记录和未归类想法。
- [reviews](../reviews/README.md)：阶段复盘。
- [archive](../archive/README.md)：旧内容归档。

## 当前专题

### Agent / LLM

- [Agent](../topics/agent/README.md)
- [Agent 上下文管理](../topics/agent/上下文管理.md)
- [Agent 安全与沙箱](../topics/agent/security/README.md)
- [提示词攻击测试方案](../topics/agent/security/提示词攻击测试方案.md)
- [Agent 提示词攻击测试流程](../topics/agent/security/Agent%20提示词攻击测试流程.md)
- [LLM](../topics/llm/README.md)
- [DeepSeek HTTP payload 结构](../topics/llm/DeepSeek%20HTTP%20payload%20结构.md)
- [QLoRA 学习资料](../topics/llm/QLoRA/README.md)

### AI Coding / 软件工程

- [AI Coding](../topics/ai-coding/README.md)
- [AI Coding 的一些初步感想](../topics/ai-coding/AI.md)
- [上下文压缩作为渐进式披露机制](../topics/ai-coding/渐进式披露.md)
- [Software Engineering](../topics/software-engineering/README.md)
- [软件工程的一些初步理解](../topics/software-engineering/软件工程.md)
- [工程程序设计经验总结](../topics/software-engineering/工程程序设计经验总结.md)

### 算法

- [聚类算法笔记](../topics/algorithms/clustering/README.md)

### 项目

- [Hermes](../projects/hermes/README.md)
- [Hermes Kanban - 多 agent 协作](../projects/hermes/kanban/多agent协作.md)

### 外部资料

- [vibe-coding-cn](../reference/repos/vibe-coding-cn/README.md)
